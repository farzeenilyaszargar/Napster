  ...
  return (
    <header className="sticky top-0 z-50 w-full">
      <div className="relative">
        <nav className="flex h-12 items-center justify-between  bg-black/60 px-6 backdrop-blur-xl md:px-10 lg:px-16 xl:px-24">
          <Link
            href="/"
          ...
          >
            <span className="relative block h-4 w-5">
              ...
              />
              <span
                className={`absolute left-0 top-[7px] h-0.5 w-5 rounded-full bg-[#555555] transition-all duration-300 ease-out ${
        ...
        <div
          className={`absolute inset-x-0 top-full origin-top transition-all duration-300 ease-out ${
            isOpen
              ? "pointer-events-auto translate-y-0 scale-y-100 opacity-100"
              : "pointer-events-none -translate-y-3 scale-y-95 opacity-0"
          }`}
        >
          <div className=" bg-black/72 shadow-[0_18px_80px_rgba(0,0,0,0.55)] backdrop-blur-3xl">
        ...
        <button
          ...
          onClick={() => setIsOpen(false)}
          className={`fixed inset-0 -z-10 bg-black/20 backdrop-blur-sm transition duration-300 ${
